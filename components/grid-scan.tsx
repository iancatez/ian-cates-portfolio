...
function smoothDampVec2(
  ...
  const targetAdj = current.clone().sub(change);
  ...
  out.copy(targetAdj.add(change.add(temp).multiplyScalar(exp)));
...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const onClick = async () => {
      ...
      if (
        ...
        (window as unknown as { DeviceOrientationEvent?: unknown })
  ...
  useEffect(() => {
    ...
    if (bloomRef.current) {
      ...
      const lm = (bloomRef.current as unknown as {
