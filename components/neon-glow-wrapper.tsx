...
// Default neon color (green) - HSL values only
...
interface NeonGlowWrapperProps {
  ...
  neonColor?: string
  /** Border/padding thickness in pixels */
  borderWidth?: number
  /** Whether the element is circular (rounded-full) */
  rounded?: boolean
}

export function NeonGlowWrapper({ 
  children,
  className, 
  neonColor = DEFAULT_NEON_COLOR,
  borderWidth = 1,
  rounded = true
}: NeonGlowWrapperProps) {
  ...
  const glowStyles = React.useMemo(() => {
    ...
    return {
      backgroundColor: `hsl(${neonColor} / ${Math.max(0.5, i)})`,
      boxShadow: `
  ...
  // Don't render glow on server to avoid hydration mismatch
  if (!isMounted) {
    return (
      <div 
        className={cn(
